...
tNode *newNode( unsigned char data, int freq);
...
// return value: 트리의 root 노드의 포인터
tNode *make_huffman_tree( int ch_freq[]){
	HEAP* H = heapCreate(256);
	for(int i=0; i<256; i++){
		tNode* node = newNode(i, ch_freq[i]);
		heapInsert(H, node);
	}
	while(H->last){
		tNode* first = heapDelete(H);
		tNode* second = heapDelete(H);
		tNode* new = newNode(0, first->freq + second->freq);
		new ->left = first;
		new->right = second;
		heapInsert(H, new);
	}
	tNode* root = heapDelete(H);
	heapDestroy(H);
	return root;
}
...
int encoding( char *codes[], FILE *infp, FILE *outfp){
    ...
    fseek(outfp, 0, SEEK_END);    // 파일 포인터를 파일의 끝으로 이동시킴
    long size = ftell(outfp);          // 파일 포인터의 현재 위치를 얻음
	size /= 8; //8bit = 1byte
...
int encoding_binary( char *codes[], FILE *infp, FILE *outfp){
    ...
    while(a != EOF){
        ...
        for(int i = 0; i < len; i++){
            ...
            if(count == 8){
                fwrite(&bin, sizeof(char), 1, outfp);
                bin = 0; count = 0;
            }
        }
		a = fgetc(infp);
