...
tNode *make_huffman_tree( int *ch_freq){
    ...
    tNode *root;
    while(heap->last){
        tNode *min1 = NULL, *min2 = NULL;
        min1 = heapDelete(heap);
        min2 = heapDelete(heap);
        
        tNode *node = newNode(0, min1->freq + min2->freq);
        node->left = min1;
        node->right = min2;
        heapInsert(heap, node);
    }
...
// strdup 함수 사용함
void traverse_tree( tNode *root, char *code, int depth, char *codes[]){
    //base condition: when it is a leaf node
...
int encoding( char *codes[], FILE *infp, FILE *outfp){
    int place = 0;
    while((place = fgetc(infp))!= EOF){
        fputs(codes[place], outfp);
        if(feof(infp)) break;
    }
    long size = 0;
