...
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity){
  ...
  if(num_point == 0){
	...
	(*num_line)+=1;
	//printf("\ncreated line! capacity= %d, num_line = %d\n",*capacity, *num_line);
	...
	return lines;
	  ...
	  for(int i=0; i < num_point; i++){
		  float dist =  distance(a, b, c, points[i]) ;
		  if(max < dist){
			max = dist;
			maxp = points[i];
		  }
	...
	t_point *s21 = (t_point *)malloc(sizeof(t_point) * num_point);
	t_point *s22 = (t_point *)malloc(sizeof(t_point) * num_point);

	int n11, n12, n21, n22; // number of points in s1, s2, respectively
	
    //printf("*********************************\nmaxpoint");
	//print_points(&maxp,1 );
	separate_points( points, num_point, p1, maxp, s11, s12, &n11, &n12);
	lines = upper_hull( s11, n11, p1, maxp, lines, num_line, capacity);
	
	separate_points( points, num_point, maxp, pn, s21, s22, &n21, &n22);
	lines = upper_hull( s21, n21, maxp, pn, lines, num_line, capacity);
	//printf("\n*********************************\n");
	free( s11); free(s12);
	free( s21); free(s22);
	return lines;
...
void separate_points( t_point *points, int num_point, t_point from, t_point to, t_point *s1, t_point *s2, int *n1, int *n2){
  ...
  *n1 = 0; *n2 = 0; 
  ...
  for(int i = 0; i<num_point; i++){
	  if(a*points[i].x + b*points[i].y < c)
	    s1[(*n1)++]= points[i];
	  else if (a*points[i].x + b*points[i].y > c)
	    s2[(*n2)++] = points[i];
  }
  //printf("\nupper");
  //print_points(s1, *n1);
